from linger_detection.yolov5.utils.general import (
    check_img_size, non_max_suppression, scale_coords, xyxy2xywh)
from linger_detection.yolov5.utils.torch_utils import select_device, time_synchronized
from linger_detection.yolov5.utils.datasets import letterbox

from linger_detection.utils_ds.parser import get_config
from linger_detection.utils_ds.draw import draw_boxes
from linger_detection.deep_sort import build_tracker
...
class VideoTracker(object):
    ...
    def run(self):
        ...
        idx_frame //= self.args.frame_interval
        is_linger = False
        for k in stranger_dict:
            if stranger_dict[k]/idx_frame > self.args.linger_thres:
                print(f'出镜率：{stranger_dict[k]/idx_frame}')
                is_linger = True
                break
        if is_linger:
